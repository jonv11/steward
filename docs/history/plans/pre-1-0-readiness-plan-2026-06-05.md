...
|------|-----------|-------------------|---------------------|-----------|------|
| ~~Cross-platform build/test/pack automation~~ | ~~A stable release needs reproducible validation on Windows, macOS, and Linux; local-only verification is not enough.~~ | `.github/workflows/ci.yml` runs build/test/pack on Windows, macOS, and Linux. **First hosted green matrix run confirmed on 2026-04-19 (v0.17.0 tag push); subsequent runs continue passing through 2026-06-05.** | **Completed** — hosted evidence confirmed through v0.17.0 | Workflow, tests, release | Closed |
| ~~Hosted GitHub Release evidence~~ | ~~A credible public pre-1.0 release path needs at least one green hosted execution of the tag-driven release workflow before maintainers rely on it.~~ | `.github/workflows/release.yml` publishes changelog-backed GitHub Releases, pushes the `Steward` package to nuget.org, and attaches the `.nupkg`, self-contained bundles, and checksums. **First hosted green run confirmed on v0.14.0; tag-driven releases confirmed green through v0.17.0.** | **Completed** — hosted evidence confirmed through v0.17.0 | Workflow, release | Closed |
...
| ~~Distribution/publication hardening~~ | ~~Packaging now works, but stable-release publication steps and verification should be explicit and repeatable.~~ | [Release process](../../project/release-process.md), [release publication checklist](../../project/release-publication-checklist.md), `CHANGELOG.md`, `.github/workflows/release.yml`, and release helper scripts now define explicit pre-1.0 release preparation, GitHub Release asset publication, and automated NuGet publication. | **Completed in v0.15.0** | Docs, release | Milestone `v0.15.0` |
...
| ~~Credible first-hour onboarding and repo-independent execution path~~ | ~~A stable release should not require README improvisation to reach first value, and cross-repo source-build usage should not depend on undocumented workarounds.~~ | README now includes a tested "First 15 Minutes" path, repo-independent source-build guidance, and explicit `global.json` hazard warnings. | **Completed in v0.16.0** | Docs, UX, packaging guidance | Milestone `v0.16.0` |
| ~~Reliable agent JSON contract baseline and mutation safety~~ | ~~Steward claims AI-agent value, so JSON mode needs a trustworthy baseline across success, failure, and mutation flows.~~ | CC-01 through CC-10 landed on the `v0.16.0` line: standard envelope mode, structured errors, `success`/exit-code separation, normalized `md query` JSON, `refactor move --apply --output json` fix, richer diagnostics/details, and broadened contract tests. | **Completed in v0.16.0** | CLI contract, code, tests, docs | Milestone `v0.16.0` |
| ~~Version and current-state story consistency~~ | ~~Stable-release readiness requires the runtime, changelog, README, and planning/current-state artifacts to describe the same version line and milestone state.~~ | Shared version metadata, changelog, README, release docs, and implementation/planning artifacts now align on `v0.16.0`. | **Completed in v0.16.0** | Planning, status, release hygiene | Milestone `v0.16.0` |
| Decide the later pre-1.0 roadmap ordering explicitly | The repo now correctly stays on `0.x`, but later pre-stable scope still needs explicit sequencing as stable criteria are defined. | User guidance now places all future work on pre-`1.0.0` milestones; [milestone-plan.md](milestone-plan-2026-06-05.md) now captures delivered lineage through `v0.17.0` plus the active `v0.18.0` follow-on work. | Yes | Planning, governance | Milestone planning update as criteria evolve |
...
| Universal JSON envelope guarantee | Standard-envelope support now exists, but the repo still needs a guarantee that every JSON-capable success and expected-failure path emits the same contract. | AI-agent contract review 2026-04-18; `v0.17.0` removed the legacy envelope mode, broadened structured errors, and deepened handoff data, but some expected-failure paths still need routing through `JsonEnvelopeWriter`. | Partial | Design, code | Milestone `v0.18.0` |
...
| Heading-level Markdown refactors (RFC-012) | Heading rename and restructure operations with cross-file reference updating. Accepted as an RFC but deferred until selector infrastructure matures. | [RFC-012](../../decisions/rfcs/RFC-012-heading-level-markdown-refactors.md) | No | Design, code | Deferred RFC |
| Governed suppressions and expiring debt (RFC-013) | Structured suppression metadata with lifecycle, ownership, and auditability. Accepted as an RFC but deferred until the pre-1.0 trust floor is established. | [RFC-013](../../decisions/rfcs/RFC-013-governed-suppressions-and-expiring-debt.md) | No | Design, code | Deferred RFC |
