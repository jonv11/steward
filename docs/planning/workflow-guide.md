...
- Link new Markdown files from a governed navigation surface (`docs/planning-index.md` or `docs/decisions/decision-index.md`). Orphaned files violate STWD-013.
- Never hand-edit `STRUCTURE.md` or `docs/decisions/decision-index.md` — they are generated by `steward maintain`.
- When your change makes an active planning or status document stale, update that document in the same change. Documentation drift is a defect.
...
- The feature is aligned with the current milestone plan or an accepted RFC/ADR.
- You have read `docs/planning/implementation-instructions.md` for current priorities.
- You have reviewed any relevant RFC or ADR for design authority.

**Steps:**

1. **Orient.** Run `steward orient --signals` and read the current implementation status.
...
6. **Update docs.** If the feature changes user-visible behavior, update `README.md`. If it changes implementation scope, update `docs/implementation-status.md`.
...
- Leaving `docs/implementation-status.md` stale after delivering milestone scope.
...
- For governed docs (under `docs/planning/`, `docs/requirements/`, `docs/decisions/`), run `steward explain path <file>` before editing to see required frontmatter and applicable rules.

**Steps:**

1. **Edit the document.** Follow existing Markdown conventions and required frontmatter schemas.
2. **Link from navigation.** Ensure the document is reachable from `docs/planning-index.md` or `docs/decisions/decision-index.md`.
...
- Hand-editing `STRUCTURE.md` or `docs/decisions/decision-index.md`.
...
1. **Create a review record** in `docs/audits/` using the naming pattern `lower-kebab-case[-YYYY-MM-DD].md`. No frontmatter is required for audit docs, but including a date in the filename is recommended.
...
3. **Link the review** from `docs/planning-index.md` under the appropriate section.
4. **Run finalization checklist.**
5. **Commit** with `chore:` or `docs:` and a descriptive message (e.g., `docs: add release-readiness assessment for v0.15.0`).
...
3. **Update the review record** with remediation status if the review is an ongoing tracking document.
...
- Review record exists in `docs/audits/` and is linked from `docs/planning-index.md`.
...
- Pre-release blockers in [pre-release-blockers.md](pre-release-blockers.md) are resolved.
...
4. **Update state docs** — `docs/implementation-status.md`, `docs/planning/milestone-plan.md`, `docs/planning/implementation-instructions.md` if the version baseline changes.
...
- Forgetting to update `docs/implementation-status.md` post-release.
...
### 9. Planning and Roadmap Update

**Objective:** Update planning artifacts — milestone plans, readiness plans, implementation instructions, backlog docs, or the planning index.

**When to use:** After scope changes, milestone completions, priority shifts, or when planning artifacts drift from reality.

**Prerequisites:**

- For new planning docs, run `steward explain path <file>` to check frontmatter requirements (`type: planning`, `status:` from allowed values).

**Steps:**

1. **Edit the planning artifact.** Keep it factual, current, and linked.
2. **Update cross-references.** If your change affects what other planning docs claim (e.g., changing milestone scope should update `milestone-plan.md` and `implementation-instructions.md`), update them in the same change.
3. **Link new docs** from `docs/planning-index.md`.
4. **Run finalization checklist.**
...
- Planning artifacts reflect current truth.
- No orphaned docs.
- `steward check` exits 0.
...
4. **Update `docs/planning/release-process.md`** if the change affects the release pipeline.
...
4. Run `steward maintain --apply` to refresh the decision index.
5. Link from `docs/planning-index.md` if the decision is architecturally significant.
...
|---------------------------|-----------------|---------------|
| New CLI command or feature | [Feature Implementation](#1-feature-implementation) | [Planning Update](#9-planning-and-roadmap-update) |
...
| Updating plans or roadmap | [Planning Update](#9-planning-and-roadmap-update) | |
...
- Consult `docs/planning/release-process.md` for the full release operator flow.
