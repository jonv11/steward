...
Hosted evidence, the remaining universal-JSON cleanup, and typed-address/config-model follow-ons remain deferred.

### `v0.18.0` — Active Next Scope

Treat the following as the active next pre-stable scope:

- capture the first hosted green CI run and the first hosted green tag-driven release run
